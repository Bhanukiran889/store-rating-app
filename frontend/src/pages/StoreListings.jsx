...
const StoreListings = () => {
    ...
    useEffect(() => {
        axios.get(`${API_BASE_URL}/api/stores/with-ratings`)
            .then(response => {
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <p className="text-slate-700 dark:text-slate-300 mb-6">Explore a list of all registered shops. Click on any shop to view its details, read reviews, and submit your own rating.</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {stores.length > 0 ? (
                    stores.map(store => (
                        <Link to={`/stores/${store.id}`} key={store.id} className="block group">
                            <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-md group-hover:shadow-xl transition-shadow duration-300 border border-slate-200 dark:border-slate-700">
                                <h3 className="text-xl font-semibold text-slate-900 dark:text-white mb-2 group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors duration-200">{store.name}</h3>
                    ...
                    <p className="text-slate-600 dark:text-slate-400 col-span-full text-center py-12">No shops found. Be the first to register one!</p>
                )}
            </div>
        </div>
    );
